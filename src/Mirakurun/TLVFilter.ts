...
interface TLVFilterOptions {
    ...
    readonly filterTlvStreamId?: number;
}
...
export default class TLVFilter extends EventEmitter {
    ...
    private _filterTlvStreamId: number | null = null;
    ...
    constructor(options: TLVFilterOptions) {
        ...
        this._filterTlvStreamId = options.filterTlvStreamId ?? null;
    ...
    private _onSDT(sdt: MHServiceDescriptionTable): void {
        ...
        if (this._filterTlvStreamId !== null) {
            // When a specific TLV stream ID is requested, accept both SDT[actual] and
            // SDT[other] but only process the table that belongs to the target stream.
            // This is necessary for hardware (e.g. PIX-SMB400) that can only tune
            // streamId=0 (all streams): every stream on the transponder emits its own
            // SDT[actual], so we must select the right one by tlvStreamId.
            if (sdt.tlvStreamId !== this._filterTlvStreamId) {
                return;
            }
        } else if (sdt.tableId !== "SDT[actual]") {
            return;
        }
        ...
        for (const service of sdt.services) {
            ...
            // because PLT may only contain packages for the current stream.
            if (this._parseSDT === false && this._serviceIds.has(service.serviceId) === false) {
                continue;
            }
